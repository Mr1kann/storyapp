...
export default class StoryDetail {
  ...
  async render() {
    return `
      <section class="container story-detail">
        ...
        <div id="bookmark-button-container" class="bookmark-container"></div>
  ...
  _showBookmarkButton(isBookmarked) {
    const container = document.getElementById("bookmark-button-container");

    if (isBookmarked) {
      container.innerHTML =
        '<button id="bookmarkButton" class="btn-danger">Unbookmark this Story</button>';
    } else {
      container.innerHTML =
        '<button id="bookmarkButton" class="btn-primary">Bookmark this Story</button>';
    }

    return document.getElementById("bookmarkButton");
  }
}
